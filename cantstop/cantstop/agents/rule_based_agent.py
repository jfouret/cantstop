...
class Rule(Enum):
  TARGET_BESTS_AND_PASS=1 # target the best 3 and pass or play randomly and replay
...
class RuleBasedAgent():
  ...
  def act(self, env : gym.Env) -> np.ndarray[np.float32]:
    if self._rng is None:
      raise AttributeError("Random generator must be initiated with reset method")
          ...
          "pair_of_dice_chosen": 
            self._rng.choice(
              info["possible_values"][
                self._rng.choice([k for k in info["possible_values"].keys()])
              ]
            ),
          "throw_dices": 1
        })
      return action
    else: AttributeError("Agent's rule not recognized")
