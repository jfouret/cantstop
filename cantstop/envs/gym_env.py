...
class GymEnv(gym.Env):
    ...
    def __init__(
        ...
        dices_one_hot: bool = True, render_mode: str = "human"
    ) -> None:
        ...
        if reward_type == RewardType.MINUS_ONE_PER_TURN:
            ...
            raise ValueError("reward_type is not Valid")
        ...
        if board_one_hot:
            ...
            current_player_progress_space = \
                gym.spaces.Box(B_LOW, B_HIGH, dtype=np.uint8)
        if dices_one_hot:
            dices_space = gym.spaces.MultiDiscrete(
                6*np.ones(4, dtype=np.uint8), dtype=np.uint8)
        else:
            dices_space = gym.spaces.Box(
    ...
    def step(self, action: np.ndarray[np.float32]
             ) -> Tuple[np.ndarray[np.float32], float, bool, dict]:
        ...
        first_value, second_value = \
            self._get_values_from_dices(action["pair_of_dice_chosen"])
        ...
        reward = self._get_reward(
            state, info, next_state, next_info, is_terminated)
    ...
    def render(self) -> None:
        if self.render_mode == "human":
            ...
            for i in range(0, self._board.shape[0]):
                ...
                y = self._board[i, ]
                # Generate the bar plot with the appropriate color from the
                # palette
                self._ax.bar(x_pos, y, width=self._bar_width,
                             color=self._cm(i), label=f"player {i}")
    ...
    def _seed(self) -> None:
        ...
        s1, s2 = \
            (self.np_random.integers(uint_max, dtype=np.uint32)
             for _ in range(2))
    ...
    def _get_reward(self, state: np.ndarray[np.float32], info: dict,
        ...
        if self._reward_type == RewardType.MINUS_ONE_PER_TURN:
            if is_terminated and \
              info["current_player_index"] == info["current_player_index"]:
                return 1
            else:
                return info["turn"] - next_info["turn"]

    def _get_values_from_dices(self,
                               pair_of_dice_chosen: int) -> Tuple[int, int]:
        return (
    ...
    def _get_info(self) -> int:
        if self._generate_info:
            return {
              ...
              "score": tuple(
                  self._get_score(i) for i in range(self._num_players))
            }
        else:
            return {
              "turn": self._turn,
    ...
    def _is_legal_value(self, value: int) -> bool:
        ...
        N = self._num_players
        max_score = np.add(
            self._board[idx], self._current_player_progress)[value] / \
            B_HIGH[value]
        if self._num_players > 1:
            max_score = max(max_score, np.max(
                np.array([self._board[(idx + i) % N][value] / B_HIGH[value]
                          for i in range(N)
                          if i != idx])
            ))
